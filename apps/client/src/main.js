...
$('nav').addEventListener('click', async event => {
  ...
  if (event.target.matches('#saved')) {
    ...
    const result = await getBookmarkList('http://localhost:3000/api/user/bookmark', { _id });
    const $content = `
    <div class="container">
    ${result
      .map(
...
$('main').addEventListener('click', async event => {
  ...
  const _id = localStorage.getItem('user_token');
  await addBookmark(`http://localhost:3000/api/user/bookmark/${event.target.getAttribute('key')}`, { _id });
